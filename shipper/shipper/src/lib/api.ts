...
const getMockDb = () => {
  if (typeof window === 'undefined') return { bookings: [], quotes: [], negotiations: [] } as any;
  const stored = localStorage.getItem('BT_SHIPPER_MOCK_DB');
  if (stored) {
  ...
  localStorage.setItem('BT_SHIPPER_MOCK_DB', JSON.stringify(defaultDb));
  return defaultDb;
};

const saveMockDb = (db: any) => {
  if (typeof window !== 'undefined') {
    localStorage.setItem('BT_SHIPPER_MOCK_DB', JSON.stringify(db));
  }
};
...
async function handleMockRequest<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (path.startsWith('/auth/')) {
    ...
    if (path.includes('/verify-otp') || path.includes('/email/login') || path.includes('/email/verify') || path.includes('/google') || path.includes('/register') || path.includes('/magic-link/verify')) {
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('bt_token', 'mock-jwt-shipper-token');
        localStorage.setItem('bt_refresh_token', 'mock-refresh-shipper-token');
      }
      return {
        access_token: 'mock-jwt-shipper-token',
        refresh_token: 'mock-refresh-shipper-token',
        is_new_user: false,
        user: {
          id: 's1',
          phone: '9876543210',
          email: 'shipper@bharattruck.com',
          full_name: 'Adani Logistics Admin',
          avatar_url: null,
          role: 'shipper'
        }
      } as any;
  ...
  if (path === '/bookings/') {
    if (method === 'POST') {
      ...
      const newBooking = {
        ...
        source_lat: body.source_lat || 18.96,
        source_lng: body.source_lng || 72.84,
        destination_address: body.destination_address,
        dest_lat: body.dest_lat || 28.56,
        dest_lng: body.dest_lng || 77.10,
        load_type: body.load_type || 'General Cargo',
        weight_kg: Number(body.weight_kg) || 5000,
        quoted_price: Number(body.quoted_price) || 20000,
        final_price: null,
        pickup_date: body.pickup_date || new Date().toISOString(),
        pickup_time_slot: body.pickup_time_slot || '09:00 - 18:00',
        status: 'pending',
  ...
  if (path.startsWith('/bookings/')) {
    ...
    // POST /bookings/:id/award
    if (path.endsWith('/award')) {
      const body = JSON.parse(options.body as string);
      const quote = db.quotes.find((q: any) => q.id === body.quote_id);
      const booking = db.bookings.find((b: any) => b.id === bookingId);
      if (booking && quote) {
        booking.status = 'accepted';
        booking.driver_id = quote.driver_id;
        booking.final_price = quote.amount;
        booking.awarded_quote_id = quote.id;
        quote.status = 'accepted';
        
        // Reject others
        db.quotes.forEach((q: any) => {
          if (q.booking_id === bookingId && q.id !== quote.id) {
            q.status = 'rejected';
          }
        });
        saveMockDb(db);
    ...
    // POST /bookings/:id/negotiation
    if (path.endsWith('/negotiation')) {
      const body = JSON.parse(options.body as string);
      const newNeg = {
  ...
  if (path.startsWith('/tracking/')) {
    if (path.includes('/route/')) {
      return { polyline: 'mock-polyline-data' } as any;
    }
    if (path.includes('/eta/')) {
      return { eta_minutes: 45, distance_km: 32 } as any;
    }
    if (path.includes('/history/')) {
      return { history: [] } as any;
    }
    if (path.includes('/track/')) {
      // Shipper read-through aggregate
      const parts = path.split('/');
