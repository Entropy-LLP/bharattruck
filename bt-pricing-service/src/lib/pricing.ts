import { z } from 'zod'
import { type VehicleClass, VEHICLE_TYPE_TO_CLASS, costBreakdown, type CostBreakdown } from './cto-cost.js'

// -----------------------------------------------------------
// Commercial split (what the shipper is charged) — unchanged v1 rate card —
// PLUS the CTO deterministic cost-breakdown anchor (Appendix A) so the shipper
// sees where the price comes from. Both are returned together.
// -----------------------------------------------------------

export const RATE_PER_KM: Record<string, number> = { mini_truck: 12, lcv: 15, hcv: 22, trailer: 35 }
...
export function computeQuote(input: QuoteInput): QuoteResult {
  ...
  return {
    ...
    currency: 'INR',
    version: 'v1-static+cto-breakdown',
